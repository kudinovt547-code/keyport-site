    ...
    <section class="kp-section" id="numbers">
      <div class="kp-narrow flex flex-col gap-12">
        <div class="flex items-center justify-between gap-6">
          ...
          <div class="text-cream-200/50 text-[11px] uppercase tracking-[0.32em]" data-reveal>фактика по проектам</div>
        </div>
          ...
          ${numbers.map(
            (n) => `
              <div class="kp-numbers__cell">
                <div class="kp-numbers__value"><em>${n.value}</em>${n.unit ? ` <span class="text-cream-200/55 text-[0.5em] align-baseline">${n.unit}</span>` : ''}</div>
